import { createSvgIcon } from "@mui/material/utils";

// 1. Вызываем функцию
const IpIcon = createSvgIcon(
  // Первый аргумент: содержимое SVG (обычно теги <path>, <circle> и т.д.)
  <svg
    width="26"
    height="26"
    viewBox="0 0 26 26"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M1 17V9C1 6.87827 1.75857 4.84344 3.10883 3.34315C4.45909 1.84286 6.29044 1 8.2 1H17.8C19.7096 1 21.5409 1.84286 22.8912 3.34315C24.2414 4.84344 25 6.87827 25 9V17C25 19.1217 24.2414 21.1566 22.8912 22.6569C21.5409 24.1571 19.7096 25 17.8 25H8.2C6.29044 25 4.45909 24.1571 3.10883 22.6569C1.75857 21.1566 1 19.1217 1 17Z"
      stroke="currentColor"
      strokeWidth="2"
    />
    <path
      d="M12.9999 17V9H15.9999C16.4773 9 16.9351 9.21071 17.2727 9.58579C17.6103 9.96086 17.7999 10.4696 17.7999 11C17.7999 11.5304 17.6103 12.0391 17.2727 12.4142C16.9351 12.7893 16.4773 13 15.9999 13H12.9999M9.3999 9V17"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </svg>,

  // Второй аргумент: Имя компонента
  "IpIcon",
);

export default IpIcon;
